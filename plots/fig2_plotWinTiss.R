...
#                       header=TRUE, sep='\t')
sig_hits = read.table("data_tables/tf_eqtls/all.16tiss.fdr05.eqtl_sig.txt",
                      header=TRUE, sep='\t') %>%
  merge(tf_info %>% select(1:2) %>% rename(tf_id = Name),
        by.x=c('tf'), by.y=c('TF')) %>%
  merge(tiss_info, by.x="tissue", by.y="TISSUE_NAME") %>%
  filter(tf_id != as.character(phenotype_id)) %>%
  mutate(tiss = factor(tissue, levels=unique(as.character(tiss_info$TISSUE_NAME))))
...
sig_hits %>%
  ...
  xlab('Tissue') +
  ylab('Within-tissue at 5% FDR') +
  geom_text(data=top_per_tiss, 
            aes(TISSUE_ABBRV,rep(c(10000,9000),8),
                label=top_tf),
            size=3) +
  scale_y_continuous(breaks=c(0,2000,4000,6000,8000, 9500),
                     labels=c("0", "2000", "4000",'6000','8000', 'Top TF'))
